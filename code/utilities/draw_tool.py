...
class DrawTool:
    ...
    def draw_coordinates(self, frame, same, color=(0, 0, 255)):
    ...
    def get_perspective_src(self, background):
        ...
        print(np.asarray(self.shapes[0])*2)

        return np.asarray(self.shapes[0])*2
    ...
    def click_event(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if self.curr == -1:
                ...
                self.curr = (x, y)
                self.shapes[len(self.shapes)-1].append(self.curr)
                if(get_distance(self.start, self.curr) > 10):
